...
volatile epfunc_t epfunc_in[STM32ENDPOINTS];
volatile epfunc_t epfunc_out[STM32ENDPOINTS];

volatile static config_pack_t setup_packet;
volatile static uint16_t lastaddr = LASTADDR_DEFAULT;
volatile static const uint8_t *ep0_buf = NULL;
volatile static uint16_t ep0_count = 0;
volatile static uint8_t configuration = 0;
...
static void ep0_out(uint8_t epnum){
  ...
  if(setup_packet.bmRequestType & 0x80){
    ...
    void *data = ZLPP;
    uint16_t size = 0;
    usb_class_ep0_in( (config_pack_t*)&setup_packet, &data, &size );
    if(setup_packet.wLength < size) size = setup_packet.wLength;
    ep0_send(data, size);
  }else{
    //---OUT---
    ...
    if(!usb_class_ep0_out( (config_pack_t*)&setup_packet, bytesread, rxcount )){
...
void usb_ep_init(uint8_t epnum, uint8_t ep_type, uint16_t size, epfunc_t func){
  ...
  if( dir_in ){
    ...
    ENDP_DTOG_TX(epnum, 0);
    // ENDP_DTOG_TX(epnum, USB_EP_DTOG_TX);
  }else{
    ...
    ENDP_DTOG_RX(epnum, 0);
    usb_epdata[epnum].rx_count = 0;
...
__attribute__((interrupt))void USB_LP_CAN1_RX0_IRQHandler(){
...
void _usb_ep_write(uint8_t idx, const uint16_t *buf, uint16_t size){
  volatile pma_descr_t *descr = &((pma_descr_t*)usb_epdata)[idx];
