...
"\r\n"
"Чтобы изменить настройки, достаточно вписать новые значения в соответствующие файлы, отмонтировать диск (ограничение съемных носителей: система может закешировать изменения и не записать их сразу), после чего нажать и удерживать кнопку SETUP. Как и при коротком нажатии, устройство переподключится в рабочем режиме (без \"флешки\"). Но при коротком нажатии изменения будут потеряны, а при длинном - сохранены. Чтобы восстановить настройки по умолчанию, подключайте его к USB при зажатой кнопке. Высветится \"настройки будут сброшены при отжатии кнопки\". Если кнопку отжать - сбросятся. Если выдернуть из USB (и потом подключить с отжатой кнопкой) - сохранятся.\r\n"
"\r\n"
